...
```{r Lectura de datos}
#######################################################################################
file_actual<-"data_orden/Datos Dia/BaseDatosInmueblesPortada20-12-2022.xls"
# Lectura de datos
...
sum(filtro_prueba)
localizaciones_mod<- tidygeocoder::geocode(data.frame(X= addres_new),'X',  method = "arcgis" )
...
```
```{r}
files <- list.files("C:/Users/ESTADISTA_2/Documents/Portada Inmobiliaria/Inmuebles_Arrendados/data_orden/Datos Dia")
file_ <- files[1]
file_
fecha<-strsplit( file_, "Portada")[[1]][2]
fecha
strsplit(fecha, ".xls" )[[1]][1]
primer <- read_excel(paste("C:/Users/ESTADISTA_2/Documents/Portada Inmobiliaria/Inmuebles_Arrendados/data_orden/Datos Dia",file_, sep = "/"))
# Seleccionar solo los sectores de interes
centro_costos_codigo<-c("01","02","03","04","08","09","10","11","12","19","22","25","26","28")
primer <- primer[is.element(primer$CentroCostos, 
                                         centro_costos_codigo ), ]
...
centro_costos_nombres<- c("Laureles","Sabaneta","Poblado",
                          ...
                          "San Antonio Pr." , "Calasanz",
                          "Fontibon")
primer$NoCCostos<-as.numeric(primer$CentroCostos)
primer$CentroCostos <- factor(primer$CentroCostos,
                                   labels = centro_costos_nombres)

primer %>% group_by(CentroCostos) %>% summarise()
```
