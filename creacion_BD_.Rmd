...
filtro<- rep(TRUE, nrow(inmuebles))
direcciones_nuevas<-inmuebles[filtro,c('Direcciones_c','Direcciones_prueba','Ciudad' )]
localizaciones_nuevas<- geocode(data.frame(X= address_nuevas[filtro]),'X',  method = "arcgis" )

direcciones_nuevas$localizaciones.lat<-localizaciones_nuevas$lat
direcciones_nuevas$localizaciones.long<-localizaciones_nuevas$long
#direcciones_nuevas<-rbind(direcciones_nuevas, localizaciones)

write.csv2(x = unique(direcciones_nuevas), file = "BD/localizacion_inmuebles.csv", row.names = F, fileEncoding = "UTF-8")
