...
inmuact_newfil[3406, 7] <- "MEDELLIN"
...
```{r, echo=FALSE, warning=FALSE,fig.align="center"}
# Gráfico de dispersión de Vr. Canon vs Valor Administración
ggplot(inmuebles_bloq, aes(x=`Vr Canon`, y=`Vr Administracion`, color=CentroCostos_n)) + geom_point()+  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ggtitle("Diagrama de dispercion entre Valor de Canon vs el Valor de Administración") +
theme (plot.title = element_text(family="Comic Sans MS",
                                  size=rel(1), #Tamaño relativo de la letra del título
                                  vjust=2, #Justificación vertical, para separarlo del gráfico
                                  face="bold", #Letra negrilla. Otras posibilidades "plain", "italic", "bold" y "bold.italic"
...
```{r}
# Crear una nueva variable con las direcciones sin modificar
inmuact_newfil$inmu_direccion <- inmuact_newfil$direcciones
```

```{r}
#Agregar los municipios de cada dirección  a inmu_direccion (OJO = SOLO CORRERLO UNA VEZ )
lugar_punto <- paste(inmuact_newfil$Ciudad, ", COLOMBIA" , sep=" ")
```

```{r}
# Agregar el municipio y pais a la variable inmu_direccion
inmuact_newfil$inmu_direccion <- paste(inmuact_newfil$inmu_direccion, lugar_punto , sep=" ")
```

```{r}
# Crear las localizaciones sin remover nada de la dirección
# Transformar las direcciones a latitud y longitud (localizaciones)
localizaciones_1 <- geo(address=inmuact_newfil$inmu_direccion, method="arcgis" ) 
```
